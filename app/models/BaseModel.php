...
class BaseModel 
{
	...
	{
        $conn = new PDO('mysql:host=127.0.0.1;dbname=php2_asm;charset=utf8', 'root', '');
        return $conn;
    }
	...
	public static function count(){
		$model = new static();
        $query = "select COUNT(*) as 'sum' from $model->tableName";
        $stmt = $model->getConnect()->prepare($query);
		$stmt->execute();
	...
	public static function countWhere($arr){
		$model = new static();
        $query = "select COUNT(*) as 'sum' from $model->tableName where $arr[0] $arr[1] '$arr[2]'";
        $stmt = $model->getConnect()->prepare($query);
		$stmt->execute();
		$result= $stmt->fetchAll(PDO::FETCH_CLASS, get_class($model));
		return $result[0];	
	...
	public static function getMax($arr){
		$model = new static();
        $query = "select MAX($arr[0]) as 'max' from $model->tableName where $arr[1] $arr[2] '$arr[3]'";
        $stmt = $model->getConnect()->prepare($query);
		$stmt->execute();
		$result= $stmt->fetchAll(PDO::FETCH_CLASS, get_class($model));
		return $result[0];	
	...
	public static function getMin($arr){
		$model = new static();
        $query = "select MIN($arr[0]) as 'min' from $model->tableName where $arr[1] $arr[2] '$arr[3]'";
        $stmt = $model->getConnect()->prepare($query);
		$stmt->execute();
		$result= $stmt->fetchAll(PDO::FETCH_CLASS, get_class($model));
		return $result[0];	
