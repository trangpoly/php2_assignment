...
<body>
    <div class="signup">
        <h1 class="title">CẬP NHẬT QUIZ</h1>
        <form action="" method="POST" class="form-group" enctype="multipart/form-data">
            <label for="name" class="signup-label">Tên quiz:</label>
            <input type="text" name="name" class="sign-input" value="<?=$quiz_detail["name"]?>">
            <label for="inputState">Môn học</label>
            <select id="inputState" name= "subject_id" class="form-control sign-input">
                <option value="<?=$quiz_detail["subject_id"]?>" selected><?=$subject_detail["name"] ?></option>
                <?php foreach($subject_All as $subject): ?>
                    <option value="<?=$subject["id"]?>"><?=$subject["name"]?></option>
                <?php endforeach?>
            </select>
            <label for="duration_minutes" class="signup-label">Thời gian làm quiz:</label>
            <input type="number" min="10" max = "60" name="duration_minutes" class="sign-input" value="<?=$quiz_detail["duration_minutes"]?>">
            <label for="exampleFormControlFile1">Thời gian mở quiz:</label> <br><?=$quiz_detail["start_time"]?>
            <input type="datetime-local" name="time_start" class="sign-input">
            <label for="exampleFormControlFile1">Thời gian đóng quiz:</label> <br><?=$quiz_detail["end_time"]?>
            <input type="datetime-local" name="time_end" class="sign-input">
            <label for="exampleFormControlFile1">Trạng thái:</label><br>
            <select id="inputState" name= "status" class="form-control sign-input">
                <option selected><?=$quiz_detail["status"]==0?"Được làm":"Không được làm"?></option>
            ...
            <button class="quiz_update" name="updateQuiz">Cập nhật quiz</button>
