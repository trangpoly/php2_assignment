...
<body>
    <div class="signup">
        <h1 class="title">CẬP NHẬT MÔN HỌC</h1>
        <form action="" method="POST" class="form-group" enctype="multipart/form-data">
        <!-- <form action="" method="POST" class="form-group" enctype="multipart/form-data"> -->
            <label for="name" class="signup-label">Tên môn học:</label>
            <input type="text" name="name" class="sign-input" value="{{$subject_update->name}}">
            <label for="desc" class="signup-label">Mô tả môn học:</label>
            <textarea name="desc" id="" class="sign-input" cols="30" rows="10">{{$subject_update->detail}}</textarea>
            <label for="exampleFormControlFile1">Ảnh đại diện:</label><br>
            <input type="text" name="image" class="sign-input" value="{{$subject_update->image}}">
            @if(isset($_SESSION['error']))
                <span class="error">{{$_SESSION['error']}}</span>          
            @unset($_SESSION['error'])
            @endif 
            <button class="signup-submit" name="login">Cập nhật</button>
